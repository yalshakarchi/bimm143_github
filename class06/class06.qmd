...
Functions are at the heart of uisng R. Everything we do involves calling and using functions (from data input, analysis to results output).
...
> Q. Write the code to genrate a random 12 nucleotide long DNA sequences?
...
> Q. Write a first version function called `generate_dna()` that generates a user specified length random `n` DNA sequences?
...
> Q. Modify you rfunction to return a FASTA like sequence so rather than [1] "G" "C" "A" "A" "T" we want "GCAAT"
...
> Q. Give the user an option to return FASTA format output sequnce or standard multi-element vector format
...
> Q. Use your new `generate_protein()` function to generate all sequnces between length 6 and 12  amino-acids in length and check of any of these are unique in nature (i.e. found in the NR database at NCBI)? 
